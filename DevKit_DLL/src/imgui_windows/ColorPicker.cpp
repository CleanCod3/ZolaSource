...
void ColorSelect::Render() {
    ...
    if (!ImGui::Begin("T0P",0, ImGuiWindowFlags_NoMove| ImGuiWindowFlags_NoTitleBar |ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoResize))
    {
    ...
    if(g_pCICPlayer)
    {
        ImGui::SetWindowPos("T0P",ImVec2(g_pCGInterface->GetCustomTitleGUI()->GetPos().x + 370,g_pCGInterface->GetCustomTitleGUI()->GetPos().y + 85));
...
void ColorNameSelect::Render() {
    ...
    int green = static_cast<int>(m_color[1] * 255.0f);
    int blue = static_cast<int>(m_color[2] * 255.0f);
    D3DCOLOR NameColor = D3DCOLOR_ARGB(255, red, green, blue);
    if (g_pCICPlayer)
    {
        ImGui::SetWindowPos("T0P", ImVec2(g_pCGInterface->GetCustomTitleGUI()->GetPos().x + 370, g_pCGInterface->GetCustomTitleGUI()->GetPos().y + 85));
        g_pCGInterface->GetCustomTitleGUI()->OnColorSelect(NameColor);
    }

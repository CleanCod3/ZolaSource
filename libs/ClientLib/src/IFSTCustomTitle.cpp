...
#define ST_CUSTOMTITLE_APPLY 31 //title custome
#define ST_CUSTOMTITLE_CLOSE_BUTTON 32 // manager tittles
#define ST_CUSTOMTITLE_NAME_COLOR 37 //namecolor
#define ST_CUSTOMTITLE_NAME_INPUT 36 // input titles
...
GFX_BEGIN_MESSAGE_MAP(CIFCustomTitle, CIFMainFrame)
                    ...
                    ONG_COMMAND(ST_CUSTOMTITLE_NAME_COLOR, &CIFCustomTitle::On_BtnClickChangeNameColor)
                    ONG_COMMAND(ST_CUSTOMTITLE_TAP2, &CIFCustomTitle::On_BtnClickTenTitle_TAB2)
                    ONG_COMMAND(ST_CUSTOMTITLE_TAP1, &CIFCustomTitle::OnClick_MauTitle_TAB1)
...
bool CIFCustomTitle::OnCreate(long ln)
{
    ...
    RECT butt1Rect = { 23,212,111,24 }; // buy turn
    RECT buttRect = { 147,212,111,24 }; // title color
    RECT butt21Rect = { 277,212,111,24 }; //name color
    RECT butt2Rect = { 407,212,111,24 }; // manage titles
    ...
    m_mybutton21 = (CIFButton*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFButton), butt21Rect, ST_CUSTOMTITLE_NAME_COLOR, 0);
    ...
    m_mybutton21->TB_Func_13("interface\\ifcommon\\com_mid_button.ddj", 0, 0);

        m_title->SetText(TSM_GETTEXTPTR("UIIT_ST_TITLE_CUSTOM"));
    m_mybutton->SetText(TSM_GETTEXTPTR("UIIT_ST_TITLE_CUSTOM_APPLY"));
    m_mybutton21->SetText(TSM_GETTEXTPTR("UIIT_ST_TITLE_CUSTOM_NAMECOLOR"));
...
void CIFCustomTitle::OnClick_Exit()
{
    ...
    {
        g_pCGInterface->ShowMessage_Notice(L"You Can't Use Manage Titles in Our Server Sorry :)");
        return;
    }
...
void CIFCustomTitle::OnClick_MauTitle_TAB1()
{

    CIFCustomTitle::ColorSelectShow = true;
    tab1= false;
    m_mybutton5->TB_Func_13("interface\\ifcommon\\com_long_tab_on.ddj", 0, 0); //Tab1
    m_mybutton6->TB_Func_13("interface\\ifcommon\\com_long_tab_off.ddj", 0, 0); //Tab2
    wchar_t buffer1[80];
...
void CIFCustomTitle::On_BtnClickTenTitle_TAB2()
{

    CIFCustomTitle::ColorSelectShow = false;
    m_custom_label3->m_FontTexture.SetColor(D3DCOLOR_RGBA(255, 0, 0, 255)); //#FF0000
    tab1= true;
    m_mybutton5->TB_Func_13("interface\\ifcommon\\com_long_tab_off.ddj", 0, 0); //Tab1
    m_mybutton6->TB_Func_13("interface\\ifcommon\\com_long_tab_on.ddj", 0, 0); //Tab2
    wchar_t buffer1[80];
...
void CIFCustomTitle::On_BtnClickChangeNameColor()
{
    if (NameColor == 0)
    {
        wchar_t message[1000];
        swprintf_s(message, TSM_GETTEXTPTR("UIIT_STT_NAMECOLOR_NOTICE"));
        CIFSystemMessage* systemmessage = reinterpret_cast<CIFSystemMessage*>(g_pCGInterface->m_IRM.GetResObj(68, 1));
        int color = D3DCOLOR_ARGB(255, 255, 0, 0);
        systemmessage->WriteMessage(0xFF, color, message, 0, 1);
    }
        ...
        buf << std::n_string("!DOIMAUNAMECOLOR");
        buf << std::n_string("!DOIMAUNAMECOLOR");
        buf << NameColor;
        SendMsg(buf);
    }
